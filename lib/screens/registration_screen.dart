                    ...
                    MyButton(buttonText: "Register", onTap: (){
                      if (textEditingController1.text.isNotEmpty && textEditingController2.text.isNotEmpty && textEditingController3.text.isNotEmpty){
                        if (textEditingController2.text == textEditingController3.text){
                          UtilityFunctions.getMyToast(message: "You are successfully registered!");
                          Navigator.pushReplacement(context, MaterialPageRoute(builder: (BuildContext context){
                            return const SelectQuizScreen();
                          }));
                        }
                        else {
                          UtilityFunctions.getMyToast(message: "Your password did not matched");
                        }
                      }
                      else {
                        UtilityFunctions.getMyToast(message: "You didn't fill all of them! Try to fill again");
                      }
                    }),
                    SizedBox(height: 25.h),
                    Text("If you have already account click here", style: MyTextStyles.inter500.copyWith(color: Colors.grey.shade400, fontSize: 16.sp),),
                  ],
                ),
              )
            ],
